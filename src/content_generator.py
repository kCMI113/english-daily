  ...
  "expressions": [
    {
      ...
      "synonyms": [{"en": "유사 표현", "ko": "한국어 뜻", "example": "짧은 영어 예문"}],
      "antonyms": [{"en": "반의 표현", "ko": "한국어 뜻", "example": "짧은 영어 예문"}],
      "examples": [
...
4. synonyms: 실제로 의미가 유사한 표현 2~3개. 각각 짧은 예문 1개 필수.
5. antonyms: 실제로 반대 의미인 표현 1~2개. 각각 짧은 예문 1개 필수. 없으면 [].
6. word_forms: type이 "word"인 경우에만 필수. 해당 단어의 품사별 변형을 포함. 동사는 "현재 / 과거 / 과거분사" 형태로. 존재하지 않는 품사는 빈 문자열. type이 word가 아니면 이 필드를 생략.
...
### word 예시 (word_forms 포함)
{
  "type": "word",
  "expression": "hesitant",
  "pronunciation": "헤지턴트",
  "meaning_ko": "망설이는, 주저하는",
  "explanation_ko": "확신이 없거나 결정을 내리지 못할 때 쓰는 표현입니다. 'I'm hesitant about...' 형태로 자주 쓰이며, 격식/비격식 모두 사용 가능합니다.",
  "word_forms": {"noun": "hesitation", "verb": "hesitate / hesitated / hesitated", "adjective": "hesitant", "adverb": "hesitantly"},
  "synonyms": [{"en": "reluctant", "ko": "꺼리는", "example": "She was reluctant to agree."}],
  "antonyms": [{"en": "decisive", "ko": "단호한", "example": "He's very decisive under pressure."}],
  "examples": [
    {"en": "I'm a bit hesitant about accepting the offer.", "ko": "그 제안을 받아들일지 좀 망설여져."},
    {"en": "Don't be hesitant — just go for it.", "ko": "망설이지 마. 그냥 해봐."}
  ]
}

### phrasal_verb 예시 (word_forms 없음)
{
  "type": "phrasal_verb",
  ...
  "synonyms": [{"en": "work out", "ko": "해결하다", "example": "We worked out a solution."}, {"en": "sort out", "ko": "정리하다", "example": "Let me sort out this mess."}],
  "antonyms": [{"en": "give up on", "ko": "포기하다", "example": "Don't give up on your dreams."}],
  "examples": [
...
class ContentGenerator:
    ...
    def generate(
        ...
        print(f"Requesting {count} expressions...")
        expressions = self._try_generate(today, difficulty, count, exclude_str)

        # Fallback: reduce to 10
        if not expressions:
            fallback = 10
            print(f"Failed. Retrying with {fallback} expressions...")
            expressions = self._try_generate(today, difficulty, fallback, exclude_str)

        if not expressions:
            raise RuntimeError("Content generation failed after all attempts")

        self._clean_foreign_chars(expressions)
        self._validate(expressions, count=0)
